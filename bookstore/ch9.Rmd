...
xbar.income<-mean(~income,data=srs.income)
```
...
Since we took a SRS, we can be fairly sure that this estimate, $\bar{x}=$ $ `r format(xbar.income, scientific=FALSE)` is *close* to the true parameter value.  However, if we re-run the code above, we will get a different value.
...
```{r imagpopsamp15, echo=FALSE}
set.seed(17563)
samples<-do(15)*mean(popsamp(50,imagpop)$income)
samples
```

Based on these samples, it seems likely that the true average annual income (the parameter value) probably falls somewhere in between $`r format(min(samples),scientific=FALSE)`$ and $`r format(max(samples),scientific=FALSE)`.$  

[How *likely* is it that the parameter falls in this range, though?  In other words, how *confident* are we that the true parameter value falls between $`r format(min(samples),scientific=FALSE)`$ and $`r format(max(samples),scientific=FALSE)`$?  When we start trying to think of this in terms of likelihood or probability, we need a more mathematical way to compute such an interval.  The method we used above is very rudimentary.  We'll get to a better method of construction momentarily!]  
...
So, $\bar{x}=$ \$`r format(xbar,scientific=FALSE)` is the sample estimate of the population mean, $\mu$.   
...
So the population mean is $\mu=$ \$`r format(mu,scientific=FALSE)`.  
...
```{r,echo=FALSE}
set.seed(138)
CI.income<-t.testGC(~income,data=mysample) #this is formula-data input
```
...
So, our 95% confidence interval for the estimation of the population mean, $\mu$, is ($`r format(CI.income$interval[1],scientific=FALSE)`,$`r format(CI.income$interval[2],scientific=FALSE)`).
...
We are lucky in this case because we are a powerful being and we actually do know the value of $\mu$.  So, we can check to see if $\mu$ is, in fact, contained in the interval we computed.  The population mean, $\mu=$ $`r format(mu,scientific=FALSE)`, so it is contained in the 95% confidence interval that was computed in Step 2.
...
for (i in 1:reps){
  ...
  mytest2 = t.testGC(~income, data = mysample2)
  mytest2
...
You should notice that *about* 95% of the 100 intervals contain $\mu=$ $`r format(mu,scientific=FALSE)`.  In other words, *about* $0.95\cdot 20=$ `r 0.95*20` intervals should contain $\mu$ and *about* $0.05\cdot 20=$ `r 0.05*20` does not.  
...
We can be reasonably sure that the average annual income of the folks from the `imagpop` population  is between $`r format(CI.income$interval[1],scientific=FALSE)` and $`r format(CI.income$interval[2],scientific=FALSE)`.
...
```{r distxbar,echo=FALSE, fig.cap="Xbar Distribution:  The distribution of the sample means approximately follows a normal distribution centered at the EV(xbar) with spread given by SD(xbar)."}
x =seq(-4,4,length=1000)
y =dnorm(x,mean=0, sd=1)
plot(x,y, type="l", lwd=1,xlab="xbar",ylab="",axes=FALSE)
axis(side=1, at=c(-3,-2,-1,0,1,2,3),labels=c("","","",expression(mu),"","",""))
```
...
```{r empruletwo,echo=FALSE, fig.cap="68-95 Rule 2:  This graph is the visualization of the second part of the 68-95 Rule for the distribution of the sample means."}
x =seq(-4,4,length=1000)
y =dnorm(x,mean=0, sd=1)
plot(x,y, type="l", lwd=1,xlab="xbar",ylab="",axes=FALSE)
axis(side=1, at=c(-3,-2,-1,0,1,2,3),labels=c("","","",expression(mu),"","",""))
...
text(x=0,y=0.15,labels="~95%",cex=1.7)
```
...
This seems to make sense, but this is not the confidence interval that the `t.testGC` function gave us originally.  The interval from `t.testGC` was ($`r format(CI.income$interval[1],scientific=FALSE)`,$`r format(CI.income$interval[2],scientific=FALSE)`).  We seem to be off by a little bit.    
...
```{r twosd, echo=FALSE,fig.cap="Two SD:  This graph shows the true area under the curve within two standard deviations of the mean."}
invisible(pnormGC(c(-2,2),region="between",mean=0,sd=1,graph=TRUE))
```

The shaded region is actually `r pnormGC(c(-2,2),region="between",mean=0,sd=1)`.  This is because 68-95 Rule is just an approximation.  Since we are looking for a 95% confidence interval, we would like our multiplier, or $z$-score, to be such that the shaded region is exactly 0.95, not `r pnormGC(c(-2,2),region="between",mean=0,sd=1)`.
...
```{r,echo=FALSE}
z.upper=qnorm(0.975,mean=0,sd=1)
```
...
```{r,echo=FALSE}
z.lower=qnorm(0.025,mean=0,sd=1)
```
...
```{r better95mult,echo=FALSE,fig.cap="Better 95% Multiplier.  1.96 works out a bit better than 2."}
invisible(pnormGC(c(-1.96,1.96),region="between",mean=0,sd=1,graph=TRUE))
```

That's better!  Let's adjust our formula for our 95% confidence interval.

$$(\bar{x}`r z.lower` \cdot SE, \mbox{ } \bar{x}+ `r z.upper`\cdot SE)$$

We can now re-compute our 95% confidence interval using this new formula:  $\mbox{CI}=(\bar{x}-1.96\cdot SE,\mbox{  } \bar{x}+1.96\cdot SE)=$ (`r format(stats.income[6] -1.96*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`, `r format(stats.income[6] +1.96*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`).

Summarizing what we've done so far:

* Using R's `t.test` function, $CI=$ ($`r format(CI.income$interval[1],scientific=FALSE)`, $`r format(CI.income$interval[2],scientific=FALSE)`).

* Using the multiplier 2 from the 68-95 Rule, $CI=$ ($`r format(stats.income[6] -2*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`, $`r format(stats.income[6] +2*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`)

* Now, using the multiplier $z=1.96$, $CI=$ ($`r format(stats.income[6] -1.96*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`, $`r format(stats.income[6] +1.96*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`)
...
```{r,echo=FALSE}
t.upper = qt(0.975, df=49)
t.lower=qt(0.025,df=49)
```
...
```{r echo=FALSE,fig.cap="t-Multiplier.   The multiplier for sample size 50 is about 2.01."}
invisible(ptGC(c(-2.01,2.01),region="between",df=49,graph=TRUE))
```

Since $t=2.01$ is the correct multiplier, let's recompute the confidence interval with it and compare to our previous calculations:  $\mbox{CI}=(\bar{x}-2.01\cdot SE,\mbox{  } \bar{x}+2.01\cdot SE)=$ (`r format(stats.income[6] - t.upper*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`, `r format(stats.income[6] +t.upper*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`)

* Using R's `t.testGC` function, $CI=$ ($`r format(CI.income$interval[1],scientific=FALSE)`, $`r format(CI.income$interval[2],scientific=FALSE)`).

* Using the multiplier 2 from the 68-95 Rule, $CI=$ ($`r format(stats.income[6] -2*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`, $`r format(stats.income[6] +2*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`)

* Using the multiplier $z=1.96$, $CI=$ ($`r format(stats.income[6] -1.96*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`, $`r format(stats.income[6] +1.96*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`)

* Now, using the multiplier $t=2.01$, $CI=$ ($`r format(stats.income[6] -t.upper*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`, $`r format(stats.income[6] +t.upper*(stats.income[7]/sqrt(stats.income[8])),scientific=FALSE)`)
...
```{r,echo=FALSE}
CI.height=t.testGC(~height,data=m111survey) #this is formula-data input
```

```{r,echo=FALSE}
t.testGC(~height,data=m111survey) 
```
...
```{r,echo=FALSE}
CI.height99=t.testGC(~height,data=m111survey,conf.level=0.99)
```
...
```{r,echo=FALSE}
CI.height68=t.testGC(~height,data=m111survey,conf.level=0.68)
```
...
```{r,echo=FALSE}
CI.sleepsex=t.testGC(sleep~sex,data=m111survey,first="female")
```
...
```{r,echo=FALSE}
CI.head=t.testGC(mean=c(55.3,57.5),sd=c(1.8,2.4),n=c(36,36))
```
...
```{r,echo=FALSE}
CI.diff=t.testGC(~ideal_ht-height,data=m111survey) 
```
...
```{r, echo=FALSE}
CI.diff.hts=t.testGC(mean=1.852667,sd=2.507909,n=30)
```
...
A reasonable difference for the average difference in height between a college woman and her mother is any number between `r format(CI.diff.hts$interval[1],scientific=FALSE)` inches and `r format(CI.diff.hts$interval[2],scientific=FALSE)` inches.  On average, college women are taller than their mothers. 
...
```{r,echo=FALSE}
CI.hd<-prop.testGC(x=c(85,24), n=c(1105,1379), conf.level=0.99)
```
