...
```{r majorsent, echo=F,fig.cap="Sentence by Major.  The survey respondents are broken into four groups, accordng to the type of majors they intend to pursue."}
...
xyplot(biomass~salt,data=saltmarsh,
       xlab="Salinity (ppm)",
       ylab="Biomass",
...
```{r jiffgvhist, echo=FALSE,fig.cap="Ratings Difference.  Most of the differences are positive, indicating that subjects usually rated the Jiff-labeled peanut butter more highly."}
histogram(~diff,main="Difference in Ratings\n(Jiff-GV)",
          xlab="Difference", type="count",
          breaks=seq(-5.5,8.5,by=1))
```
...
```{r knifegunstripplot, echo=FALSE,fig.cap="Knife or Gun?  Apparently those slain by Knife yell louder!"}
stripplot(volume~means,data=knifeorgunblock,
         main="Yelling While Being Slain",sub="Means of Slaying",
         panel= function(...){
